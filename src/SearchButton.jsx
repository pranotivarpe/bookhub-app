...
export default function BookSearch() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(url); // Fetch book data from Google Books API
      if (response.ok) {
        const data = await response.json(); // Parse response data
        if (data.items && data.items.length > 0) {
          const book = data.items[0]; // Get the first book from the response
          const bookData = {
          ...
          setBookInfo(bookData); // Set the book information
